...
export default function Restaurant(props) {
    ...
    const addFavorite = () => {
        if (!userLogged) {
            toastRef.current.show("Para usar el sistema de favoritos, debes estar logueado.");
            ...
            db.collection("favorites")
                ...
                .catch(() => {
                    toastRef.current.show("Error al añadir el restaurantea favoritos");
                })
        }
    };

    const revomeFavorite = () => {
        db.collection("favorites")
            .where("idRestaurant", "==", restaurant.id)
            .where("idUser", "==", firebase.auth().currentUser.uid)
            .get()
